<!-- <script lang="ts">
	...
	// selectedOption = choices.find((option) => option === value);
	// if (selectedOption) {
	// 	index = choices.indexOf(selectedOption);
...
<span class={className + 'label'}>
	...
	<select class="select bg-slate-100" id={identifier} bind:value={selected} name={identifier}>
		<option value="" disabled selected>Selecciona</option>
		{#each choices as choice (choice)}
			<option value={JSON.stringify(choice)}>{choice.valor}</option>
		{/each}
	</select>
</span> -->
